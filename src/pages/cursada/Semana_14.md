---
layout: src/layouts/PostCursadaLayout.astro
title: Semana 14

inicio: 2026-06-29

descripcion: Semana de recuperatorios de ambos parciales para todas las comisiones.

horarios:
  - Comision: 1
    Dia: Viernes 3 de julio
    Modalidad: PRESENCIAL
    Hora: 8.00hs
    Aula: A determinar #LAB MA-113
    Edificio: A determinar #MALVINAS ARGENTINAS (PA)

  - Comision: 2
    Dia: Martes 30 de junio
    Modalidad: PRESENCIAL
    Hora: 18.00hs
    Aula: LAB MA-113
    Edificio: MALVINAS ARGENTINAS (PA)

  - Comision: 3
    Dia: Miércoles 1 de julio
    Modalidad: PRESENCIAL
    Hora: 18.00hs
    Aula: A determinar #LAB LP-206
    Edificio: A determinar #La Patria

  - Comision: 4
    Dia: Viernes 3 de julio
    Hora: 18.00hs
    Modalidad: PRESENCIAL
    Aula: A determinar #LAB TA-001
    Edificio: A determinar #TRABAJO ARGENTINO

  - Comision: 5
    Dia: Miércoles 1 de julio
    Modalidad: PRESENCIAL
    Hora: 18.00hs
    Aula: A determinar #LAB LP-207
    Edificio: A determinar #La Patria

  - Comision: 6
    Dia: Viernes 3 de julio
    Hora: 18.00hs
    Modalidad: PRESENCIAL
    Aula: A determinar #LAB TA-002
    Edificio: A determinar #TRABAJO ARGENTINO

  - Comision: 7
    Dia: Viernes 3 de julio
    Hora: 18.00hs
    Modalidad: PRESENCIAL
    Aula: A determinar #LAB MA-111
    Edificio: A determinar #MALVINAS ARGENTINAS (PA)

  - Comision: Todas
    Dia: Sábado 4 de julio
    Modalidad: VIRTUAL
    Hora: 10.00hs
    URL: https://meet.google.com/sia-cweg-zen

  #Tutorías
  - Comision: Todas
    Dia: Lunes 29 de junio
    Modalidad: 📣 VIRTUAL (tutoría) 📣
    Hora: 18.00hs
    URL: https://meet.google.com/sia-cweg-zen

ejercicios:
  - name: RECUPERATORIO - COMISIÓN 2
    urlTemplate: https://github.com/obj1-unahur/recu1y2_com2_2026c1 #completar con plantilla correcta
    destOrg: obj1-unahur-2026s1
    comentarios:
      - name: Examen recuperatorio para comisión 2
    isPrivate: true
    visible: false

  - name: RECUPERATORIO - COMISIÓN 3
    urlTemplate: https://github.com/obj1-unahur/recuperatorioCom3_2026C1 #completar con plantilla correcta
    destOrg: obj1-unahur-2026s1
    comentarios:
      - name: Examen recuperatorio para comisión 3
    isPrivate: true
    visible: false
    requireTOTP: true

  - name: RECUPERATORIO - COMISIÓN 5
    urlTemplate: https://github.com/obj1-unahur/ #completar con plantilla correcta
    destOrg: obj1-unahur-2026s1
    comentarios:
      - name: Examen recuperatorio para comisión 5
    isPrivate: true
    visible: false
---

- Esta semana se tomará examen recuperatorio tanto del primer parcial como del segundo, para cada comisión en su día y horario correspondiente. La modalidad es PRESENCIAL, pero la forma de evaluación puede variar según decida el/la docente de cada comisión (consultar cualquier duda).

- **Temas a evaluar**

  - • Para recuperar primer parcial: Objeto, mensaje, polimorfismo, colecciones, bloques, referencias y test unitarios.

  - • Para recuperar segundo parcial: super() + clases y herencia (todo lo anterior y además clases y herencia).

- **¿Tengo que recuperar?**

  - • Si tenés dudas de si te conviene recuperar y qué, volvé a leer el <a href="/contrato-pedagogico#aprobacion">Contrato pedagógico, sección aprobación</a>.

- **Recordatorios y recomendaciones importantes**

  - • Quienes tengan notebook con puerto y cable de red y wollok funcionando, se sugiere llevarla para rendir y ayudar a que todes tengan pc para trabajar; considerar llevar también los adaptadores y accesorios que requieran para conectarse, teniendo en cuenta que la red WiFi puede ser inestable.

  - • El examen es INDIVIDUAL y ABIERTO, lo que significa que pueden utilizar todos los apuntes que tengan tanto físicos como digitales.

  - • Llevar anotado el token personal de github. Asegurarse de que esté vigente.
